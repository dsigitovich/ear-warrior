...
export class AnalogSynthEngine {
  ...
  private setupSignalChain (): void {
    ...
    // Connect drift LFO to synth (we'll handle detune differently for polyphonic synth)
  ...
  updateParams (params: Partial<SynthParams>): void {
    ...
    if (params.attack !== undefined || params.decay !== undefined ||
      ...
      this.synth.set({
        envelope: {
          ...
          sustain: params.sustain ?? 0.4,
          release: params.release ?? 0.8
        }
      })
    }
    ...
    if (params.filterEnvAmount !== undefined) {
      this.envelope.baseFrequency = 200 + (params.filterEnvAmount * 3800)
    }
